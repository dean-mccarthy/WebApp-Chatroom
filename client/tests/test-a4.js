						...
						try {
							...
							if (initialized){
								...
								try {
									...
									console.log('merged:' + merged + ', response:' + chatResponse );
						...
						try {
							...
							try {
								...
								if (!isEquivalent(testMessages, receivedMessages)){
									result.comments.push(printError(`The ${testMessages.length} test messages sent by the test WebSocket client does not match the ${receivedMessages.length} messages received`, '\nSent:', testMessages, '\nReceived:', receivedMessages));
								}
								else {
									...
									if (added && added.messages instanceof Array){
										if (!isEquivalent(testMessages, added.messages.slice(roomMessages.length).map(item => Object.assign(item, { roomId: __tester.defaults.testRoomId })))){
											result.comments.push(printError(`The ${testMessages.length} test messages sent by the test WebSocket client does not match the messages in the last conversation object`));
										}
										else {
											result.score += 1;
